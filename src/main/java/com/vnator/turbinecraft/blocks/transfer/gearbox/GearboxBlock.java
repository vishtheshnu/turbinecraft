...
public class GearboxBlock extends Block{
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        /*
        if(player.getHeldItem(handIn).getToolTypes().contains(ToolType.get("wrench")) && !worldIn.isRemote){
            System.out.println(player.isSneaking());
            ((GearboxTile)worldIn.getTileEntity(pos)).onWrench(hit.getFace(), player.isSneaking());
